...
function SidebarItem({ icon, label, active, theme = 'dark', onClick }: { icon: React.ReactNode, label: string, active?: boolean, theme?: 'light' | 'dark', onClick?: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 h-12 px-5 rounded-xl transition-all group border ${active
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-2 fancy-scrollbar">
          {sessions.length === 0 ? (
            ...
            sessions.map((s) => (
              <div
                key={s.id}
                className={`p-4 rounded-xl text-xs leading-relaxed border cursor-pointer transition-all flex items-center justify-between group/item ${activeChatId === s.id
